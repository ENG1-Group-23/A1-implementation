...
public class TitleScreen extends ScreenAdapter{
    ...
    public TitleScreen(HeslingtonHustle game) {
        this.game = game;
    }
    ...
    public void render(float delta) {
        //show text on the screen for instructions
        Gdx.gl.glClearColor(0, .25f, 0, 1);
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
        game.batch.begin();
        game.font.draw(game.batch, "Title Screen!", Gdx.graphics.getWidth() * .25f, Gdx.graphics.getHeight() * .75f);
        game.font.draw(game.batch, "You have one week to study for exams.", Gdx.graphics.getWidth() * .25f, Gdx.graphics.getHeight() * .5f);
